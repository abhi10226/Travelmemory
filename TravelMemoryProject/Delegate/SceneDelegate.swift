...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        ...
        //        let navigation = UINavigationController(rootViewController: WidgetVC())
        //        window.rootViewController = navigation
        //        self.window = window
        if maybeOpenedFromWidget(urlContexts: URLContexts) {
    ...
    private func maybeOpenedFromWidget(urlContexts: Set<UIOpenURLContext>) -> Bool {
        guard let _: UIOpenURLContext = urlContexts.first(where: { $0.url.scheme == "widget-deeplink" }) else { return false }
        return true
